...
public class ExchangeClient {
	...
	public static class ExchangeClientBuilder {
		...
		/**
		 * The hostname of the Exchange Web Service. It will be used for connecting with
		 * URI https://hostname/ews/exchange.asmx
		 *
		 ...
		 *
		 * @param domain the domain of the Active Directory. The first part of the
		 *               username. For example: MYDOMAIN\\username, set the MYDOMAIN.
		 * @return the builder for chain usage.
		 */
		...
		/**
		 * The username of the MS Exchange Smtp Server. The second part of the username.
		 * For example: MYDOMAIN\\username, set the username.
		 *
		...
		/**
		 * You can specify a list with email addresses that will be used as cc for this
		 * email send.
		 *
		 * @param recipientCc the list with email addresses that will be used as cc for
		 *                    this email send.
		 * @return the builder for chain usage.
		 */
		...
		/**
		 * You can specify a list with email addresses that will be used as bcc for this
		 * email send.
		 *
		 * @param recipientBcc the list with email addresses that will be used as bcc
		 *                     for this email send.
		 * @return the builder for chain usage.
		 */
	...
	public boolean sendExchange() throws Exception {
		...
		if (attachments != null && attachments.size() > 0) {
			for (Map.Entry<String, MultipartFile> fileEntry : attachments.entrySet()) {
				try {
					emailMessage.getAttachments().addFileAttachment(fileEntry.getKey(),
							fileEntry.getValue().getBytes());
				} catch (Exception ex) {
