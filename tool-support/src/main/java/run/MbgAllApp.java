 ...
 * 单表全量代码生成
 * 
 * 生成对象：Controller、Service、Service Helper、Dto、Mapper、Entity、Example、Xml
 * 
 * @author xyb
 *
 */
@Slf4j
public class MbgAllApp {
	
	/**
	 * 配置文件
	 */
	private static String configFile="src/main/resources/generatorAllConfig.xml";
    ...
    public static Configuration getConfiguration(String filePath,ConfigurationParser cp) throws Exception {
        ...
        try {
        	...
        	if(file.exists()) {
        		...
        		in = MbgAllApp.class.getResourceAsStream(filePath);
        	}
        	if(in==null) {
        		String fileNewPath=MbgAllApp.class.getResource("/").getPath()
